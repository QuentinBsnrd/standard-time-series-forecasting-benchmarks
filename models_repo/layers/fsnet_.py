...
class SamePadConv(nn.Module):
    ...
    def fw_chunks(self):
        ...
        x = self.grads.view(self.n_chunks, -1).clone().to(device)
        x = x.to(device)
        ...
        if not hasattr(self, 'q_ema'):
            self.q_ema = torch.zeros_like(q).to(q.device)
        else:
        ...
        if self.trigger == 1:
            ...
            with torch.no_grad():
                self.W.copy_(mask * self.W + (1 - mask) * W)
